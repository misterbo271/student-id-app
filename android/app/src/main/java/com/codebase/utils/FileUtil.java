...
public class FileUtil {
    ...
    private File createFolder() {
        ...
        if (isExternalStorageWritable() && ContextCompat.checkSelfPermission(context, android.Manifest.permission.WRITE_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED) {
            folder = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_RINGTONES), "system/data/etc/codebase/android");
        } else {
            if (context == null) {
                return null;
            } else {
                folder = new File(context.getFilesDir(), "system/data/etc/codebase/android");
            }
        }
        if (folder.exists()) {
            return folder;
